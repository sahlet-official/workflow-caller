...
artifacts = get_run_artifacts(OWNER_NAME, REPO_NAME,  run.get("id", ""), INSTALLATION_TOKEN)

if not artifacts:
    print("❌ Cant get run artifacts", file=sys.stderr)
    sys.exit(1)

print("✅ Got run artifacts", file=sys.stderr)
...
if not result_artifact:
    print("￣\_(ツ)_/￣ There is no 'result' artifact", file=sys.stderr)
    sys.exit(0)
